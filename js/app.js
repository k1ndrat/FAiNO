(() => {
    ...
    (() => {
        ...
        popup.addEventListener("click", (e => {
            let target = e.target;
            console.log(target);
            if (target.closest(".tabs__title._blue")) popup.classList.add("_blue"); else if (target.closest(".tabs__title")) popup.classList.remove("_blue");
        }));
        document.querySelector(".tasks-section__create-btn");
        const createBody = document.querySelector(".tasks-section__create");
        const infoBody = document.querySelector(".tasks-section__info");
        const tasks = document.querySelector(".list-tasks");
        document.addEventListener("click", (e => {
            let target = e.target;
            if (createBody && tasks && infoBody) {
                if (target.closest(".create-task__cancel-btn") || !target.closest(".tasks-section__create") && !target.closest(".tasks-section__create-btn")) {
                    createBody.classList.remove("_show");
                    if (!target.closest(".tasks-section__random-btn")) tasks.classList.remove("_tiny");
                } else {
                    if (closeAndShowWindowsTimeout) clearTimeout(closeAndShowWindowsTimeout);
                    createBody.classList.add("_show");
                    tasks.classList.add("_tiny");
                }
                let meet = target.closest(".list-tasks__task");
                if (meet) {
                    try {
                        tasks.querySelector(".list-tasks__task._active").classList.remove("_active");
                    } catch (error) {}
                    meet.classList.add("_active");
                } else if (!target.closest(".tasks-section__info")) try {
                    tasks.querySelector(".list-tasks__task._active").classList.remove("_active");
                } catch (error) {}
                if (meet || target.closest(".tasks-section__info")) {
                    tasks.classList.add("_tiny");
                    infoBody.classList.add("_show");
                } else if (!target.closest(".tasks-section__random-btn")) {
                    tasks.classList.remove("_tiny");
                    infoBody.classList.remove("_show");
                }
                let formCheck = target.closest("form.swiper-slide__body");
                if (formCheck) {
                    formCheck.classList.toggle("checked");
                    let checkBox = formCheck.querySelector("input");
                    formCheck.classList.contains("checked") ? checkBox.value = "true" : checkBox.value = "false";
                }
                if (target.closest(".tasks-section__random-btn")) getRandomMeet();
                document.querySelector(".create-task__dates");
                let startTime = document.querySelector("select[name='startTime']").value;
                let endTime = document.querySelector("select[name='endTime']").value;
                let infoDate = document.querySelector(".selectedDay").innerHTML;
                if (target.closest(".create-task__time-btn")) if (startTime && endTime && infoDate) {
                    console.log(startTime);
                    console.log(endTime);
                    console.log(infoDate);
                    addNewDate(infoDate, startTime + " - " + endTime);
                }
            }
        }));
        let rand;
        let randClassesTimeout;
        let closeAndShowWindowsTimeout;
        function getRandomMeet() {
            if (randClassesTimeout) clearTimeout(randClassesTimeout);
            let allMeets = tasks.querySelectorAll(".list-tasks__task");
            if (rand) {
                let newRand = Math.floor(Math.random() * allMeets.length);
                while (rand == newRand) newRand = Math.floor(Math.random() * allMeets.length);
                rand = newRand;
            } else rand = Math.floor(Math.random() * allMeets.length);
            console.log(rand);
            allMeets.forEach((meet => {
                meet.classList.remove("random");
                meet.classList.remove("_active");
            }));
            allMeets[rand].classList.add("random");
            randClassesTimeout = setTimeout((() => {
                allMeets.forEach((meet => {
                    meet.classList.remove("_active");
                }));
                allMeets[rand].classList.remove("random");
                allMeets[rand].classList.add("_active");
            }), 1e3);
            window.history.pushState("meet", "Title", `?meet=${rand}`);
            smoothScroll(allMeets[rand]);
            closeAndShowWindowsTimeout = setTimeout((() => {
                infoBody.classList.add("_show");
                tasks.classList.add("_tiny");
            }), 1e3);
        }
        ...
        function addNewDate(date, time) {
            const taskDates = document.querySelector(".create-task__dates");
            taskDates.classList.add("_hide");
            const dateEl = taskDates.querySelector(`[data-date='${date}']`);
            if (!dateEl) {
                const li = document.createElement("li");
                li.classList.add("dates__item");
                li.setAttribute("data-date", date);
                const button = document.createElement("button");
                button.setAttribute("type", "button");
                button.setAttribute("data-spoller", "");
                button.classList.add("dates__title");
                button.classList.add("_spoller-active");
                const p = document.createElement("p");
                p.textContent = date;
                const img = document.createElement("img");
                img.setAttribute("src", "img/icons/arrow-up.svg");
                img.setAttribute("alt", "arrow-ico");
                button.appendChild(p);
                button.appendChild(img);
                li.appendChild(button);
                const ul = document.createElement("ul");
                ul.classList.add("dates__body");
                const li1 = document.createElement("li");
                li1.classList.add("dates__time");
                li1.setAttribute("data-time", time);
                li1.textContent = time;
                ul.appendChild(li1);
                li.appendChild(ul);
                taskDates.appendChild(li);
            } else {
                const ul = dateEl.querySelector(".dates__body");
                const timeEl = taskDates.querySelector(`[data-date='${date}'] [data-time='${time}']`);
                if (!timeEl) {
                    const li1 = document.createElement("li");
                    li1.classList.add("dates__time");
                    li1.setAttribute("data-time", time);
        ...
        const renderCalendar = () => {
            ...
            document.querySelector(".selectedDay").innerHTML = (new Date).toDateString();
            let days = "";
            for (let x = firstDayIndex; x > 0; x--) days += `<div class="prev-date">${prevLastDay - x + 1}</div>`;
            for (let i = 1; i <= lastDay; i++) if (i === (new Date).getDate() && date.getMonth() === (new Date).getMonth() && date.getFullYear() === (new Date).getFullYear()) days += `<div class="day active">${i}</div>`; else days += `<div class="day">${i}</div>`;
        ...
        if (monthDays) {
            ...
            document.querySelector(".next").addEventListener("click", (() => {
                date.setMonth(date.getMonth() + 1);
                if (date.getMonth() == 0) console.log(date.getFullYear());
                renderCalendar();
            }));
            monthDays.addEventListener("click", (e => {
                if (e.target.classList.contains("day")) {
                    ...
                    document.querySelector(".selectedDay").innerHTML = new Date(`${date.getFullYear()}-${date.getMonth() + 1}-${e.target.innerText}`).toDateString();
                }
            }));
            renderCalendar();
        }
